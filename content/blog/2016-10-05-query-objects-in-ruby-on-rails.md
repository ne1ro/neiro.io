...
Database queries are common when you develop web applications. _Ruby on Rails_
and it's _ActiveRecord_ liberates you from writing tons of boilerplate SQL code and results in creation of elegant, eloquent queries in plain Ruby.
...
_Bad cases of using ActiveRecord queries_
...
These problems can be solved by using _Query Object_ pattern — a common
technique that isolates your complex queries.

_Query Object_ in ideal case is a separate class that contains one specific
query that implements just one business logic rule.

### Implementation:

For most of the cases _Query Object_ is PORO that accepts relation in
constructor and defines queries named like an _ActiveRecord_ common methods:
...
If your scope relates to existing _QueryObject_, you can easily extend its
...
_Query Objects_ should be devised to support composition with other _Query
Objects_ and other ActiveRecord relations. In the example below two composed
...
- Methods of Query Object are named like _ActiveRecord_ methods (_all, last,
  count, etc_)
